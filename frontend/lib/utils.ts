...
export const formatNumberWithDecimal = (num: number): string => {
  // Always return a string with exactly two decimal places (e.g., 49.00, 49.50)
  const fixed = Number(num).toFixed(2);
  // Remove trailing zeros beyond two decimals shouldn't happen because toFixed(2) guarantees two decimals
  return fixed;
};
